...
class PeerLinkDelayMeasurement(Scene):
    def construct(self):
        ...
        self.wait(3)
        self.play(Uncreate(t1), Uncreate(t2))
        ...
        t1 = MathTex(r"\operatorname{neighborRateRatio}", r" = {t_{3_N} - t_{3_0} \over t_{4_N} - t_{4_0}}", font_size=36)
        t2 = MathTex(r"\operatorname{meanLinkDelay}", r" = \operatorname{neighborRateRatio}", r" \cdot {(t_{4_N} - t_{1_N}) - (t_{3_N} - t_{2_N}) \over 2}", font_size=26)
        t3 = MathTex(r"= {t_{3_N} - t_{3_0} \over t_{4_N} - t_{4_0}}", r" \cdot {(t_{4_N} - t_{1_N}) - (t_{3_N} - t_{2_N}) \over 2}", font_size=26)
        group = VGroup(t1, t2, t3).arrange(DOWN, aligned_edge=RIGHT).to_edge(RIGHT)
        # group = VGroup(t1, t2).arrange(DOWN, aligned_edge=RIGHT)
        # group.next_to(config.right_side, LEFT)
        self.play(Write(t1))
        self.next_section()
        self.play(Succession(Write(t2), Write(t3)))
        self.wait(3)
